...
M584 W0.0                ; Map W axis to mainboard driver 0
M569 P0.0 S1             ; Motor direction (invert if needed)
M906 W1500 I30           ; 1.5A motor current, 30% idle current
M92 W200                 ; Placeholder value - depends on mechanics
M350 W16 I1              ; 16x microstepping with interpolation
M201 W800                ; Acceleration [mm/s^2]
M203 W10000              ; Maximum speed [mm/min]
M566 W400                ; Jerk speed [mm/min]


; Endstops and Probes 
;-------------------------------------------------------------------------------
M574 X1 S1 P"^1.io0.in"  ; 3HC X homing position X1 = axis min, S1 = switch type
M574 Y2 S1 P"^1.io1.in"  ; 3HC Y homing position Y1 = axis min, S1 = switch type
M574 U1 S1 P"^0.io1.in"  ; Mainboard U homing position.
                         ; U1 = axis min, S1 = switch type
M574 V1 S1 P"^0.io3.in"  ;Endstop pipette
M574 W1 S1 P"^0.io4.in"  ;Endstop gripper

M574 Z0                  ; Configure z switch as a Z probe, not as an endstop. 
M558 P8 C"io0.in" H10 F360 T6000 ; H = dive height
                                 ; F = probe speed
                                 ; T = travel speed
...
M208 X0:330 Y0:400 Z0:300 U0:70 V0:450 W0:300             

;For U axis Set Elastic Lock (U axis) max rotation angle
